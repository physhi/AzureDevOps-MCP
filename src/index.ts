  ...
  try {
    ...
    allowedTools.has("listProjects") && server.tool("listProjects", 
      "List all projects",
      {
        ...
        top: z.coerce.number().optional().describe("Maximum number of projects to return"),
        skip: z.coerce.number().optional().describe("Number of projects to skip")
      },
      async (params, extra) => {
    ...
    allowedTools.has("getRepository") && server.tool("getRepository", 
      "Fetch comprehensive details for a specific Git repository by its name or ID within a project. Returns repository metadata including name, project, size, default branch, remote URL, and web URL. Supports both repository names (e.g., 'MyProject') and repository IDs (GUIDs).",
      {
        ...
        repository: z.string().describe("The repository name (e.g., 'MyProject') or ID (GUID) to retrieve details for. Repository names are case-insensitive and must be unique within the project.")
      },
      async (params, extra) => {
    ...
    allowedTools.has("listBranches") && server.tool("listBranches", 
      "List all branches in a Git repository with optional name pattern filtering and pagination. Returns branch details including name, commit ID, and object ID. Results can be filtered using wildcards (e.g., 'feature/*' for all feature branches). Supports both repository names and IDs.",
      {
    ...
    allowedTools.has("searchCode") && server.tool("searchCode", 
      "Search for files and code across repositories based on text content and file extensions. This performs a full-text search across repository contents. For best results, use specific search terms and optionally narrow the scope to a specific repository or file type.",
      {
        ...
        top: z.coerce.number().optional().describe("Maximum number of search results to return. Use this to limit large result sets for common search terms.")
      },
      async (params, extra) => {
    ...
    allowedTools.has("getFileContent") && server.tool("getFileContent",
      "Retrieve the content of a specific file from a Git repository with line numbers (arrow notation) for easy LLM comprehension. Returns formatted content with comprehensive metadata including line ranges, file size, and truncation status. Content is automatically truncated to 200 lines or 8K characters (whichever comes first) for optimal context usage. Use startLine and lineCount parameters to retrieve specific ranges for large files. Optionally specify a branch, tag, or commit to retrieve file content from a specific version. Supports both repository names and IDs.",
      {
        ...
        startLine: z.coerce.number().optional().describe("Starting line number (1-based) to begin reading from. Default: 1. Use this to navigate through large files by requesting specific ranges (e.g., startLine=201 for lines starting at 201)."),
        lineCount: z.coerce.number().optional().describe("Number of lines to return starting from startLine. Default: all lines, Maximum: 200 lines per request. Use this with startLine to retrieve specific sections of large files (e.g., lineCount=100 to get 100 lines)."),
        versionDescriptor: z.object({
    ...
    allowedTools.has("getCommitHistory") && server.tool("getCommitHistory",
      "Retrieve the commit history for a Git repository with metadata (ID, author, date, message, file changes). Returns a chronological list of commits with summary statistics at the top. Optionally filter to commits affecting a specific file path and use pagination to handle repositories with extensive history. Supports both repository names and IDs.",
      {
        ...
        top: z.coerce.number().optional().describe("Maximum number of commits to return in the response. Use this to limit results for repositories with extensive history."),
        skip: z.coerce.number().optional().describe("Number of commits to skip before starting to return results. Use with 'top' for implementing pagination through commit history."),
        projectId: z.string().optional().describe("The project ID or name to filter repositories by. If omitted, uses the default project from configuration.")
      },
      async (params, extra) => {
    ...
    allowedTools.has("createPullRequest") && server.tool("createPullRequest", 
      ...
      {
        repository: z.string().describe("The repository name (e.g., 'MyProject') or ID (GUID) where the pull request will be created. Repository names are case-insensitive."),
    ...
    allowedTools.has("createTestDataGenerator") && server.tool("createTestDataGenerator", 
      "Generate test data for automated tests",
      {
        ...
        recordCount: z.coerce.number().optional().describe("Number of records to generate")
      },
      async (params, extra) => {
    ...
    allowedTools.has("runSecurityScan") && server.tool("runSecurityScan", 
      "Run security scans on repositories. Supports both repository names and IDs.",
      {
        repository: z.string().describe("Repository name (e.g., 'MyProject') or ID (GUID) to scan. Repository names are case-insensitive."),
    ...
    allowedTools.has("getPackageVersions") && server.tool("getPackageVersions", 
      "Get versions of a package in a feed",
      {
        ...
        top: z.coerce.number().optional().describe("Maximum number of versions to return")
      },
      async (params, extra) => {
    ...
    allowedTools.has("getContainerImageTags") && server.tool("getContainerImageTags", 
      "Get tags for a container image",
      {
        ...
        top: z.coerce.number().optional().describe("Maximum number of tags to return")
      },
      async (params, extra) => {
    ...
    allowedTools.has("suggestCodeOptimization") && server.tool("suggestCodeOptimization", 
      "Suggest code optimizations using AI. Supports both repository names and IDs.",
      {
        ...
        lineStart: z.coerce.number().optional().describe("Starting line number"),
        lineEnd: z.coerce.number().optional().describe("Ending line number"),
    ...
    allowedTools.has("identifyCodeSmells") && server.tool("identifyCodeSmells", 
      "Identify potential code smells and anti-patterns. Supports both repository names and IDs.",
      {
        repository: z.string().describe("Repository name (e.g., 'MyProject') or ID (GUID). Repository names are case-insensitive."),
    ...
    allowedTools.has("getBuilds") && server.tool("getBuilds",
      "List builds with optional filters (status, result, branch, definition, tags)",
      {
        ...
        tagFilters: z.preprocess(coerceArray, z.array(z.string()).optional()).describe("Filter by tags"),
        top: z.coerce.number().optional().describe("Maximum number of builds to return (default 25)"),
    ...
    allowedTools.has("getDefinitions") && server.tool("getDefinitions",
      "List pipeline/build definitions with optional filters",
      {
        ...
        top: z.coerce.number().optional().describe("Maximum number of definitions to return (default 25)"),
